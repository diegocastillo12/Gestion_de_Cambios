...
class CronogramaModel {
  ...
  async create(data) {
    const { idProyecto, idFase, nombre, descripcion, fechaInicio, fechaFin, esReportable, idEntregable } = data;
    const sql = `
  ...
  async update(idActividad, data) {
    const { idFase, nombre, descripcion, fechaInicio, fechaFin, esReportable, idEntregable, estado } = data;
    const sql = `
      UPDATE cronograma_actividades
      SET id_fase = ?, nombre = ?, descripcion = ?, fecha_inicio = ?, fecha_fin = ?,
          es_reportable = ?, id_entregable = ?, estado = ?
      WHERE id_actividad = ?
    `;
    return query(sql, [
      idFase || null,
